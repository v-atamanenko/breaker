...
#define	GAME_BallRelease_TimeOut	(FPS_1S * 4)		// Lacher des balles après x secondes d'inactivité.
...
void InitLevel(u32 nLevel)
{
	...
	if (gBreak.nLevel == LEVEL_Max - 1)
	{
		...
		// Rajoute Doh !
		gBreak.nPlayerPosY = SCR_Height - 17;		// *** patch, ne devrait pas servir, uniquement là pour précalcul correct des angles quand on commence sur le niveau du boss (en debug).
...
void BreakerInit(u32 nTransitInit)
{
	...
	if (nTransitInit)
		...
		SDL_BlitSurface(gVar.pLevel, NULL, gVar.pScreen, NULL);		// Copie de l'image de fond. => Patch pour corriger le bug de la pause pendant la sélection du level (pendant 1 frame, ombre des briques sur le décor précédent).
...
void Brk_MoveBall(void)
{
	...
	for (i = 0; i < BALL_MAX_NB; i++)
	{
		...
		if (pBall->nUsed)
		{
			...
			{
				// Balle en mouvement.
				gVar.nInactiveCnt = 0;		// RAZ compteur d'inactivité.
...
//			pBall->nPosY >= (gBreak.nPlayerPosY - (s32)pBall->nRayon) << 8)
			...
			pBall->nSpr, pBall->nPosX >> 8, pBall->nPosY >> 8))
		{
			...
			u32	nBatSfx = e_Sfx_BatPing;	// Sfx par défaut.
...
*/
		...
		if (gBreak.nPlayerPosX - pSpr->nPtRefX <= WALL_XMax + 8)
		{
			// On force le déplacement dans la warp zone.
			gBreak.nPlayerPosX++;
	...
	// On relâche les balles après x secondes d'inactivité.
...
//todo: éventuellement, faire une vitesse de dépl max. / si last - pos > vit max, pos = last pos +- vmax.
	...
	// Si nécessaire, on force le déplacement vers la warp zone.
	if (gBreak.nPlayerFlags & PLAYER_Flg_GoToWarpZone)
	{
		if (t != gBreak.nPlayerPosX)
			gBreak.nPlayerFlags &= ~PLAYER_Flg_GoToWarpZone;	// Si le joueur a bougé, il reprend la main.
		else
			gBreak.nPlayerPosX += 4;							// Sinon on déplace le Vaus vers la sortie.
	}
...
//gBreak.nLevel = LEVEL_Max - 1;	// tst
		...
		// Au bout de x secondes d'inactivité, si pas de choix, on repart au menu.
		// (Pas de conflit avec le clic d'au dessus, parce que si clic, le compteur est remis à zéro !).
	...
	case e_Game_LevelSelectionTimeOut:		// Time out sur la sélection du niveau => on repart au menu.
		// Affichage de la phrase de selection.
		LevelSelection_Display();
	...
	case e_Game_Normal:			// Jeu.
		...
		if (gBreak.nRemainingBricks == 0)
		{
			...
			MstDoorROpen();
			// Le joueur peut continuer pour buter les monstres restants si ça le chante, mais ne peut plus mourir.
